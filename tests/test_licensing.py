...
Parts of the builder are derived from CPython's `dataclasses`, so the
PSF license and the notice of changes must ship *with the distribution*,
not merely sit in the repository. Getting that wrong is silent: the
build succeeds and the file is simply absent.
"""
...
def test_every_declared_license_file_is_at_the_root(name: str) -> None:
    # A subdirectory pattern (`LICENSES/*.txt`) is accepted by the config
    # and then silently ignored by every setuptools before 77 -- the
    # build succeeds and the file never reaches the wheel.
